...
/// seen in a CTF Exchange trade event (OrderFilled / OrdersMatched), so `map_user_trades`
/// can skip blocks a specific wallet never traded in — far more selective than a
/// contract-level key (a given wallet trades in a tiny fraction of blocks).
///
...
/// of a CTF Exchange trade (OrderFilled / OrdersMatched). Returns empty for any log
/// not from this package's contract — so an OrderFilled with the same topic0 emitted
/// by an unrelated contract cannot produce trader keys. Pulled out of `index_traders`
/// so the per-log decode logic is unit-testable without building a full `eth::Block`.
fn trader_keys_for_log(log: &eth::Log) -> Vec<String> {
    use abi::ctf_exchange::events::*;

    if !is_exchange_contract(log) {
        return Vec::new();
    }
...
mod index_tests {
    ...
    fn test_trader_keys_for_our_contract_order_filled() {
        ...
        let log = order_filled_log(CTF_EXCHANGE_CONTRACT_ADDRESS, maker, taker);
